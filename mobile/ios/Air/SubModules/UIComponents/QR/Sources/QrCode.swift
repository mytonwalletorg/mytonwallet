        ...
        return (size, { arguments in
            ...
            context.withContext { c in
                ...
                func drawMarker(x: CGFloat, y: CGFloat) {
                    var path = UIBezierPath(roundedRect: CGRect(x: x + squareSize.width / 2.0, y: y + squareSize.width / 2.0, width: markerSide, height: markerSide), cornerRadius: markerSide / 2)
                    c.addPath(path.cgPath)
                    c.strokePath()
                    
                    let dotSide = markerSide - squareSize.width * 3.0
                    path = UIBezierPath(roundedRect: CGRect(x: x + squareSize.width * 2.0, y: y + squareSize.height * 2.0, width: dotSide, height: dotSide), cornerRadius: dotSide / 2)
                    c.addPath(path.cgPath)
                    c.fillPath()
                }
