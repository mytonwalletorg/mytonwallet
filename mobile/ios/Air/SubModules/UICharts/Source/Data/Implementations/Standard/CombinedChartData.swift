...
open class CombinedChartData: BarLineScatterCandleBubbleChartData
{
    ...
    {
        assertionFailure("removeEntry(entry, dataSetIndex) not supported for CombinedData")
        return false
    }
    
    open override func removeEntry(xValue: Double, dataSetIndex: Int) -> Bool
    {
        assertionFailure("removeEntry(xValue, dataSetIndex) not supported for CombinedData")
        return false
    }
