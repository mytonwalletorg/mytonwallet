...
package org.mytonwallet.app_air.uicomponents.widgets.material.bottomSheetBehavior;
...
public class BottomSheetBehavior<V extends View> extends CoordinatorLayout.Behavior<V>
    ...
    public BottomSheetBehavior(@NonNull Context context, @Nullable AttributeSet attrs) {
        ...
        setDraggableOnNestedScroll(true);
    ...
    public void onStopNestedScroll(
        ...
        if (lastNestedScrollDy > 0) {
            ...
            if (fitToContents) {
                ...
                int currentTop = child.getTop();
                boolean isScrollingUp = velocityTracker != null && velocityTracker.getYVelocity() < 0;
                int velocityFactor = isScrollingUp ? 0 : max(0, (int) getYVelocity()) * 2;
                if (currentTop < halfExpandedOffset ||
                    currentTop - halfExpandedOffset + velocityFactor < Math.abs(currentTop - collapsedOffset)) {
                    targetState = STATE_HALF_EXPANDED;
                } else {
                    targetState = STATE_COLLAPSED;
                }
